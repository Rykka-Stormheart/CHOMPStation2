...
/obj/machinery/power/damaged_reactor // Damaged Reactor object used as the "thing" we interact with
	...
	var/ten_notif_played = FALSE					// This is false by default - procs will update as needed. This controls if we've already played our 10s countdown prior to a state ending.
	var/can_aggro = TRUE								// This is true by default - this means the mobs can aggro. Reduces constantly spamming aggro on reactor and allows our defendors to pull aggro.
	...
	var/overheat_radius = 2							// How many tiles away from the reactor do we check for solid barriers preventing us from dissipating heat?
...
/obj/machinery/power/damaged_reactor/proc/defense_mode() // Do these things EVERY tick of SSProcess - if you need it to run once, put it in an if statement, or put it in the start/complete procs.
	...
	check_clearance(overheat_radius)
...
/obj/machinery/power/damaged_reactor/proc/warmup() // Do these things EVERY tick of SSProcess - if you need it to run once, put it in an if statement, or put it in the start/complete procs.
	check_clearance(overheat_radius) // Check if we're clear and we have space around us.

	if((world.time >= (warmup_complete - 11 SECONDS)) && (ten_notif_played == FALSE)) // At or below 11s left, and haven't played our sound yet?
...
/obj/machinery/power/damaged_reactor/proc/rollback_wave()
	...
	ten_notif_played = FALSE // Reset this because we're rolling back the wave.
...
/obj/machinery/power/damaged_reactor/proc/verifying() // Do these things EVERY tick of SSProcess - if you need it to run once, put it in an if statement, or put it in the start/complete procs.
	check_clearance(overheat_radius) // Check if we're clear and have space around us.

	if(world.time >= (verification_timeout - 11 SECONDS) && (ten_notif_played == FALSE)) // At or below 11s left, and haven't played our sound yet?
		playsound(get_turf(src), countdown_sound, 100) // Play our sound locally
		ten_notif_played = TRUE // Set this true so we don't play this every time it's called
...
/obj/machinery/power/damaged_reactor/proc/complete_verification()
	...
	verification_timeout = null // Set this to null to clear the timer for re-use.
	ten_notif_played = FALSE // Reset this because we're finished with verif.
...
/obj/machinery/power/damaged_reactor/proc/check_clearance(var/tiles) // This proc is called by the defense_mode() and warmup() procs - what this does is check if we've got space around us, in x amount of tiles. This is to prevent explorers from walling us in and getting a 'free' win.
	/*
	for(var/obj/blocker in range(tiles))
		if(blocker.density)
			overheating = TRUE // Set us to TRUE
	*/
...
/obj/machinery/power/damaged_reactor/proc/aggro_mobs()
	if(can_aggro)
		for(var/obj/effect/spawner/wave_spawner/spawner in spawners)
			for(var/mob/living/M in spawner.spawned_mobs)
...
/obj/effect/spawner/wave_spawner/Destroy() // When the object is destroyed. Shouldn't be possible, but in case it's required for server aaa
	STOP_PROCESSING(SSobj, src)
